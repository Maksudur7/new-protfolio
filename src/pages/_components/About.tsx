...
import { useGlobal, useCollection } from "@/lib/api.ts";
...
export default function About() {
  const { data: aboutData, isLoading: isAboutLoading } = useGlobal("about-section");
  const { data: expData, isLoading: isExpLoading } = useCollection("experiences", "sort=order&limit=2");

  if (isAboutLoading || isExpLoading) {
    return (
      <section
        id="about"
        className="py-10 px-4 flex items-center justify-center min-h-[500px]"
      >
  ...
  const identityBulletPoints = aboutData?.identityBulletPoints || [];
        ...
        <div className="mb-16 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] items-center">
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-slate-600 dark:text-slate-300 leading-7">
              {introParagraphs.map((p: any, idx: number) => (
                ...
                <div className="relative flex flex-col items-center justify-center gap-6">
                  <div className="w-52 h-52 rounded-full border border-border bg-gradient-to-br from-primary/20 to-transparent p-1 shadow-[0_0_50px_rgba(139,92,246,0.15)]">
                    <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-full bg-muted">
                      <span className="text-6xl font-bold text-foreground">
                        <img
                          src="https://i.ibb.co.com/HT76bKkk/maksudur-rahaman.png"
                          alt=""
                        />
                      </span>
                    </div>
                  ...
                  <div className="text-center">
                    <p className="text-sm uppercase tracking-[0.3em] text-muted-foreground">
                      {aboutData?.identityLabel}
                    </p>
                    <h2 className="mt-3 text-3xl font-semibold text-foreground">
                      {aboutData?.identityHeading}
                    </h2>
                    <div className="mt-4 space-y-2 text-left text-sm text-muted-foreground">
                      <p>{aboutData?.identityLocation}</p>
                      {identityBulletPoints.map((point: any, idx: number) => (
                        <p key={idx}>{point.point}</p>
                      ))}
                    </div>
                  </div>
                </div>
          ...
          <Card className="border border-border bg-card/95 shadow-sm">
            <CardContent className="p-6">
              ...
              <ul className="space-y-4 text-sm text-slate-600 dark:text-slate-300">
                {expData?.docs?.map((exp: any, index: number) => (
                  <li key={index}>
